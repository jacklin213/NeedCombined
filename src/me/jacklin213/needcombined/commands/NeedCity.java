...
public class NeedCity implements CommandExecutor {
	...
	public NeedCity(NeedCombined instance) {
		plugin = instance;
	}

	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		// Get stuff from config
		boolean enableCommand = plugin.getConfig().getBoolean("Commands.NeedCity.Enable");
		String bmessage = plugin.getConfig().getString("Commands.NeedCity.Message");
		int cdTime = (getCooldownTime() * 20);
		String chatPluginName = msgHandler.getChatPluginName();
		// Actual command
		if (commandLabel.equalsIgnoreCase("needcity")){
			if (sender.hasPermission("needcombined.needcity")){
				if (enableCommand){
	...
	public int getCooldownTime(){
		int cdTime;
		try {
			cdTime = Integer.parseInt(plugin.getConfig().getString("Commands.NeedCity.Cooldown"));
			return cdTime;
