...
public class MyNeed implements CommandExecutor {
	...
	public MyNeed(NeedCombined instance) {
		plugin = instance;
	}

	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		// Get stuff from config
		boolean enableCommand = plugin.getConfig().getBoolean("CustomCommand.MyNeed.Enable");
		String bmessage = plugin.getConfig().getString("CustomCommand.MyNeed.Message");
		int cdTime = (getCooldownTime() * 20);
		String chatPluginName = msgHandler.getChatPluginName();
		// Actual command
		if (commandLabel.equalsIgnoreCase("myneed")){
			if (sender.hasPermission("needcombined.myneed")){
				if (enableCommand){
	...
	public int getCooldownTime(){
		int cdTime;
		try {
			cdTime = Integer.parseInt(plugin.getConfig().getString("CustomCommand.MyNeed.Cooldown"));
			return cdTime;
