...
public class NeedFaction implements CommandExecutor {
	...
	public NeedFaction(NeedCombined instance) {
		plugin = instance;
	}

	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		// Get stuff from config
		boolean enableCommand = plugin.getConfig().getBoolean("Commands.NeedFaction.Enable");
		String bmessage = plugin.getConfig().getString("Commands.NeedFaction.Message");
		int cdTime = (getCooldownTime() * 20);
		String chatPluginName = msgHandler.getChatPluginName();
		// Actual command
		if (commandLabel.equalsIgnoreCase("needfaction")){
			if (sender.hasPermission("needcombined.needfaction")){
				if (enableCommand){
	...
	public int getCooldownTime(){
		int cdTime;
		try {
			cdTime = Integer.parseInt(plugin.getConfig().getString("Commands.NeedFaction.Cooldown"));
			return cdTime;
