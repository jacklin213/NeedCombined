...
public class NeedClan implements CommandExecutor {
	...
	public NeedClan(NeedCombined instance) {
		plugin = instance;
	}

	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		// Get stuff from config
		boolean enableCommand = plugin.getConfig().getBoolean("Commands.NeedClan.Enable");
		String bmessage = plugin.getConfig().getString("Commands.NeedClan.Message");
		int cdTime = (getCooldownTime() * 20);
		String chatPluginName = msgHandler.getChatPluginName();
		// Actual command
		if (commandLabel.equalsIgnoreCase("needclan")){
			if (sender.hasPermission("needcombined.needclan")){
				if (enableCommand){
	...
	public int getCooldownTime(){
		int cdTime;
		try {
			cdTime = Integer.parseInt(plugin.getConfig().getString("Commands.NeedClan.Cooldown"));
			return cdTime;
