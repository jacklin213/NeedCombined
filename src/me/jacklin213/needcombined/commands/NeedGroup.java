...
public class NeedGroup implements CommandExecutor {
	...
	public NeedGroup(NeedCombined instance) {
		plugin = instance;
	}

	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		// Get stuff from config
		boolean enableCommand = plugin.getConfig().getBoolean("Commands.NeedGroup.Enable");
		String bmessage = plugin.getConfig().getString("Commands.NeedGroup.Message");
		int cdTime = (getCooldownTime() * 20);
		String chatPluginName = msgHandler.getChatPluginName();
		// Actual command
		if (commandLabel.equalsIgnoreCase("needgroup")){
			if (sender.hasPermission("needcombined.needgroup")){
				if (enableCommand){
	...
	public int getCooldownTime(){
		int cdTime;
		try {
			cdTime = Integer.parseInt(plugin.getConfig().getString("Commands.NeedGroup.Cooldown"));
			return cdTime;
