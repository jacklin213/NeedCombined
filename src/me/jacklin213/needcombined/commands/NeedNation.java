...
public class NeedNation implements CommandExecutor {
	...
	public NeedNation(NeedCombined instance) {
		plugin = instance;
	}

	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		// Get stuff from config
		boolean enableCommand = plugin.getConfig().getBoolean("Commands.NeedNation.Enable");
		String bmessage = plugin.getConfig().getString("Commands.NeedNation.Message");
		int cdTime = (getCooldownTime() * 20);
		String chatPluginName = msgHandler.getChatPluginName();
		// Actual command
		if (commandLabel.equalsIgnoreCase("neednation")){
			if (sender.hasPermission("needcombined.neednation")){
				if (enableCommand){
	...
	public int getCooldownTime(){
		int cdTime;
		try {
			cdTime = Integer.parseInt(plugin.getConfig().getString("Commands.NeedNation.Cooldown"));
			return cdTime;
